...
struct BMP 
{
    ...
    int Width () const { return bmp_info_header.width;  };
    int Height() const { return bmp_info_header.height; };
    ...
    void set_pixel(uint32_t x0, uint32_t y0, float R, float G, float B)
    {
    ...
    int get_pixel(uint32_t x0, uint32_t y0)
    {
        ...
        return (0xff << 24) | (R << 16) | (R << 8) | B;
    }

    const float c_inv_maxuint = 1.f / 255.f;
    void get_pixel(uint32_t x0, uint32_t y0, float &R, float &G, float &B)
    {
        uint32_t channels = bmp_info_header.bit_count / 8;
        uint32_t offset = channels * (y0 * bmp_info_header.width + x0);

        B = data[offset + 0] * c_inv_maxuint;
        G = data[offset + 1] * c_inv_maxuint;
        R = data[offset + 2] * c_inv_maxuint;
    }
